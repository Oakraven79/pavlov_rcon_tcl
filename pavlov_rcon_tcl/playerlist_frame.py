...
class PlayerListFrame:
    ...
    def update_player_frame(self, data, items_list):
        ...
        for refresh_item_dict in data:
            ...
            if unique_id in seen_unique_ids_list:
                ...
                logger.warning(
                    "Saw Unique_ID {} 2+ times in a player list, skipping this player..."
                )
            else:
                if unique_id in current_player_ids_list:
                    ...
                    if player_label_frame_obj is not None:
                        self.update_single_player_frame(
                            player_label_frame_obj, player_info, items_list
                        )
                    else:
                        logger.warning(
                            "Tried to update a player frame that has been deleted. Skipping "
                        )
                else:
                    self.player_frame_dict[unique_id] = self.create_single_player_frame(
                        player_info, items_list
                    )
            # Mark this as seen
        ...
        for unique_id in current_player_ids_list:
            if unique_id not in seen_unique_ids_list:
                ...
                if player_label_frame_obj is not None:
                    ...
                    logger.warning(
                        "Tried to delete a player frame that has been already deleted. Skipping "
                    )
    ...
    def create_single_player_frame(self, data_dict, items_list):
        ...
        main_frame = tk.LabelFrame(
            self.parent_frame.scrollable_frame,
            text="{} (Click name to view profile in browser)".format(
                data_dict["UniqueId"]
            ),
            bd=5,
        )
        ...
        main_frame.grid_rowconfigure(
            0, weight=1
        )  # only 1 row, and it can expand as needed

        main_frame.player_name_label = HoverButton(
            main_frame,
            text=data_dict["PlayerName"],
            command=lambda: self.loop.create_task(
                self.view_player_profile(data_dict["UniqueId"])
            ),
            padx=5,
            pady=2,
        )

        main_frame.player_name_label.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE), width=30
        )
        main_frame.player_name_label.grid(
            row=0, column=0, sticky="nesw", pady=2, padx=5
        )

        main_frame.grid_columnconfigure(
            0, weight=1
        )  # We want player name to expand, default weight is 0
        ...
        main_frame.player_kda_label.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3), width=20
        )
        main_frame.player_kda_label.grid(row=0, column=1, sticky="nesw", pady=2, padx=5)

        main_frame.player_cash_label = tk.Label(
            main_frame,
            text="Cash: ${}\nScore: {}".format(data_dict["Cash"], data_dict["Score"]),
        )
        main_frame.player_cash_label.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3), width=20
        )
        main_frame.player_cash_label.grid(
            row=0, column=2, sticky="nesw", pady=2, padx=5
        )

        main_frame.player_team_label = tk.Label(
            main_frame,
            text="Team: {}".format(
                data_dict["TeamId"].replace("0", "Blue").replace("1", "Red")
            ),
        )
        main_frame.player_team_label.config(font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3))
        main_frame.player_team_label.grid(
            row=0, column=3, sticky="nesw", pady=2, padx=5
        )

        # Switch teams
        main_frame.move_player_label_frame = tk.LabelFrame(
            main_frame, text="Switch Team", bd=5, borderwidth=3
        )
        main_frame.move_player_label_frame.grid(
            row=0, column=4, sticky="nesw", pady=2, padx=5
        )

        main_frame.move_player_label_frame.team_0_button = HoverButton(
            main_frame.move_player_label_frame,
            text="Blue",
            command=lambda: self.loop.create_task(
                self.button_switch_team(data_dict["UniqueId"], 0)
            ),
            padx=5,
            pady=2,
        )
        main_frame.move_player_label_frame.team_0_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.move_player_label_frame.team_0_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )
        main_frame.move_player_label_frame.team_1_button = HoverButton(
            main_frame.move_player_label_frame,
            text="Red",
            command=lambda: self.loop.create_task(
                self.button_switch_team(data_dict["UniqueId"], 1)
            ),
            padx=5,
            pady=2,
        )
        main_frame.move_player_label_frame.team_1_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.move_player_label_frame.team_1_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        # Give money button
        main_frame.give_money_label_frame = tk.LabelFrame(
            main_frame, text="Give Money", bd=5, borderwidth=3
        )
        main_frame.give_money_label_frame.grid(
            row=0, column=5, sticky="nesw", pady=2, padx=5
        )
        main_frame.grid_columnconfigure(
            5, weight=1
        )  # We want give money  to expand, default weight is 0

        main_frame.give_money_label_frame.give_money_button = HoverButton(
            main_frame.give_money_label_frame,
            button_colour="lime green",
            text="+$1000",
            command=lambda: self.loop.create_task(
                self.button_give_money(data_dict["UniqueId"], 1000)
            ),
            padx=5,
            pady=2,
        )
        main_frame.give_money_label_frame.give_money_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.give_money_label_frame.give_money_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )
        main_frame.give_money_label_frame.give_more_money_button = HoverButton(
            main_frame.give_money_label_frame,
            button_colour="lime green",
            text="+$5000",
            command=lambda: self.loop.create_task(
                self.button_give_money(data_dict["UniqueId"], 5000)
            ),
            padx=2,
            pady=2,
        )
        main_frame.give_money_label_frame.give_more_money_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.give_money_label_frame.give_more_money_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        # Item Selector
        main_frame.give_item_label_frame = tk.LabelFrame(
            main_frame, text="Give Item", bd=5, borderwidth=3
        )
        main_frame.give_item_label_frame.grid(
            row=0, column=6, sticky="nesw", pady=2, padx=5
        )
        main_frame.grid_columnconfigure(
            6, weight=1
        )  # We want give item to expand, default weight is 0
        ...
        main_frame.give_item_label_frame.selected_item = ttk.OptionMenu(
            ...
            selected_give_item,
            *items_list,
            style="player_frame.TMenubutton"
        )
        main_frame.give_item_label_frame.selected_item.configure(width=20)
        main_frame.give_item_label_frame.selected_item["menu"].configure(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE)
        )
        main_frame.give_item_label_frame.selected_item.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        main_frame.give_item_label_frame.give_item_button = HoverButton(
            main_frame.give_item_label_frame,
            text="Give Item",
            command=lambda: self.loop.create_task(
                self.button_give_item(
                    data_dict["UniqueId"],
        ...
        main_frame.give_item_label_frame.give_item_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.give_item_label_frame.give_item_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        # Switch Skin
        main_frame.switch_skin_label_frame = tk.LabelFrame(
            main_frame, text="Set Player Skin", bd=5, borderwidth=3
        )
        main_frame.switch_skin_label_frame.grid(
            row=0, column=7, sticky="nesw", pady=2, padx=5
        )
        main_frame.grid_columnconfigure(
            7, weight=1
        )  # We want switch skin  to expand, default weight is 0
        ...
        main_frame.switch_skin_label_frame.selected_item["menu"].configure(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE)
        )

        main_frame.switch_skin_label_frame.selected_item.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        main_frame.switch_skin_label_frame.give_item_button = HoverButton(
            main_frame.switch_skin_label_frame,
            text="Switch!",
            command=lambda: self.loop.create_task(
                self.button_switch_skins(
                    data_dict["UniqueId"],
        ...
        main_frame.switch_skin_label_frame.give_item_button.config(
            font=(MENU_FONT_NAME, MENU_FONT_SIZE - 3)
        )
        main_frame.switch_skin_label_frame.give_item_button.pack(
            side="left", fill=tk.BOTH, expand=tk.YES
        )

        # Kill Player
        main_frame.kill_button = HoverButton(
            main_frame,
            text="Kill Player",
            button_colour="pale green",
            command=lambda: self.loop.create_task(
                self.button_kill_player(data_dict["UniqueId"])
            ),
            padx=5,
            pady=0,
        )
        ...
        # Kick player
        main_frame.kick_button = HoverButton(
            main_frame,
            text="Kick Player",
            button_colour="orange",
            command=lambda: self.loop.create_task(
                self.button_kick_player(data_dict["UniqueId"])
            ),
            padx=5,
            pady=0,
        )
        ...
        main_frame.ban_button = HoverButton(
            ...
            button_colour="red",
            command=lambda: self.loop.create_task(
                self.button_ban_player(data_dict["UniqueId"])
            ),
            padx=5,
            pady=0,
        )
    ...
    def update_single_player_frame(self, label_frame_obj, data_dict, items_list):
        ...
        label_frame_obj.player_name_label["text"] = data_dict.get(
            "PlayerName", "Unknown"
        )
        ...
        label_frame_obj.player_team_label["text"] = "Team: {}".format(
            data_dict["TeamId"].replace("0", "Blue").replace("1", "Red")
        )
        ...
        if hasattr(self, "current_items_list"):
            if self.current_items_list != items_list:
                ...
                label_frame_obj.give_item_label_frame.selected_item.set_menu(
                    *items_list
                )
                if len(items_list) > 0:

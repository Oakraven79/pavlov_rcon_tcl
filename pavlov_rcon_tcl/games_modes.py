...
try:
    ...
    if not isinstance(GAME_MODES, dict):
        logger.error(
            "First element of game_modes.json must be a dictionary/map starting '{' - defaulting to internal list"
        )
        raise Exception("Bad game modes.json!")
    logger.info("Game modes found in file: {}".format(GAME_MODES))
except:
    logger.info(
        "Unable to load game_modes.json, reverting to internal default. Please check file and format. "
    )

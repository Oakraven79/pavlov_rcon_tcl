...
class AsyncApp(tk.Tk):
    ...
    def give_the_app_some_style(self):
        ...
        style.configure(
            "player_frame.TMenubutton",
            font=(MENU_FONT_NAME, "{}".format(MENU_FONT_SIZE), "normal"),
        )
        # set the drop down options in the server frame
        style.configure(
            "server_frame.TMenubutton",
            font=(MENU_FONT_NAME, "{}".format(MENU_FONT_SIZE), "normal"),
        )
    ...
    async def run_rcon_updates(self, interval=5):
        ...
        try:
            await asyncio.gather(
                *[
                    rcon_server_frame.exec_rcon_update()
    ...
    def add_new_server_frame(
        ...
        :return:
        """
        single_frame = SingleServerFrame(
            master=self.tabbed_server,
            loop=self.loop,
            rcon_host=rcon_host,
            rcon_port=rcon_port,
            rcon_pass=rcon_pass,
